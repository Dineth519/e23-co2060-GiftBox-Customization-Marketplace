...
states = """  const [clientSecret, setClientSecret] = useState(null);
...
"""
content = content.replace("const [toastMessage, setToastMessage] = useState(null);", "const [toastMessage, setToastMessage] = useState(null);\n" + states)
...
content = re.sub(r'const handleSaveDraft = \(\) => {.*?};', draft_methods, content, flags=re.DOTALL)
