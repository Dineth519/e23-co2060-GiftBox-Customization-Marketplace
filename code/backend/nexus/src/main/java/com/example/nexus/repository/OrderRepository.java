...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(value = "SELECT SUM(oi.quantity * oi.unit_price * 0.9) FROM orders o JOIN order_items oi ON o.order_id = oi.order_id JOIN products p ON oi.product_id = p.id WHERE p.vendor_id = :vendorId AND DATE(o.created_at) = CURDATE() AND o.status != 'CANCELLED'", nativeQuery = true)
    Double getRevenueTodayByVendorId(@Param("vendorId") Integer vendorId);

    // Dashboard — weekly orders + revenue (last 7 days)
    @Query(value = "SELECT DATE_FORMAT(o.created_at, '%a') as day, COUNT(DISTINCT o.order_id) as orders, SUM(oi.quantity * oi.unit_price * 0.9) as revenue FROM orders o JOIN order_items oi ON o.order_id = oi.order_id JOIN products p ON oi.product_id = p.id WHERE p.vendor_id = :vendorId AND o.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) GROUP BY DATE(o.created_at), DATE_FORMAT(o.created_at, '%a') ORDER BY DATE(o.created_at)", nativeQuery = true)
    List<Object[]> getWeeklyOrdersAndRevenue(@Param("vendorId") Integer vendorId);

    // Dashboard — monthly revenue (last 7 months)
    @Query(value = "SELECT DATE_FORMAT(o.created_at, '%b') as month, SUM(oi.quantity * oi.unit_price * 0.9) as revenue FROM orders o JOIN order_items oi ON o.order_id = oi.order_id JOIN products p ON oi.product_id = p.id WHERE p.vendor_id = :vendorId AND o.status != 'CANCELLED' AND o.created_at >= DATE_SUB(NOW(), INTERVAL 7 MONTH) GROUP BY YEAR(o.created_at), MONTH(o.created_at), DATE_FORMAT(o.created_at, '%b') ORDER BY YEAR(o.created_at), MONTH(o.created_at)", nativeQuery = true)
