...
const BoxBuilderPage = () => {
  ...
  // States for fetching live catalog items
  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      ...
      try {
        ...
        console.warn("Catalog fetch fallback to cart context.", err);
      } finally {
  ...
  const subtotal = useMemo(() => {
    return Object.entries(selectedItems).reduce((sum, [id, qty]) => {
      const product = availableItems.find(p => p.productId === parseInt(id, 10));
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderPayload = {
      ...
      recipientName, 
      wrappingStyle,
      items: Object.entries(selectedItems).map(([id, qty]) => ({ productId: parseInt(id, 10), quantity: qty }))
    };
        ...
        <aside className="bb-presentation-panel">
          ...
          <div className="bb-receipt-card-ledger">
            ...
            {Object.entries(selectedItems).map(([id, qty]) => {
              const p = availableItems.find(prod => prod.productId === parseInt(id, 10));
              if (!p) return null;
              return (
