  ...
  return (
    <section className="asm-issues" aria-labelledby="ai-title">
      ...
      <section className="ai-panel" aria-labelledby="ai-list-title">
        ...
        {visible.length > 0 && <div className="ai-table-scroll" tabIndex={0} role="region" aria-label="Orders needing attention"><table><thead><tr><th scope="col">Order</th><th scope="col">Issue</th><th scope="col">Items received</th><th scope="col">Status</th><th scope="col">Action</th></tr></thead><tbody>{visible.map(order => {
          const badItems = order.workspace?.items?.filter(item => item.condition === 'damaged' || item.condition === 'incorrect') || [];
          const defaultText = badItems.length > 0 
            ? `Contains ${badItems.length} ${badItems.length === 1 ? 'item' : 'items'} marked as damaged or incorrect.`
            : 'This order needs review before continuing.';
            
          return (<tr key={order.id}>
          <td><strong>#{order.id}</strong><small>{order.occasion}  {order.box} box</small></td>
          <td className="ai-description">{order.issue?.trim() || defaultText}</td>
          ...
          <td><span className={'ai-badge' + (order.status === 'hold' ? ' ai-badge-hold' : '')}>{STATUS[order.status]?.label || order.status}</span></td>
