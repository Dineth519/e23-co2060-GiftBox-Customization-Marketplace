...
export function canSubmit(state) {
  return state.receiptConfirmed && receiptsReady(state) && !state.issue && (state.status === 'assembling' || state.status === 'ready') && state.checks.length === CHECKS.length && state.checks.every(Boolean);
}
