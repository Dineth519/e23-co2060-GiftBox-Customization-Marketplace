...
import { filterOrders } from './overviewData';
...
export default function AssemblerCompleted() {
  ...
  return <section className="asm-issues" aria-labelledby="ac-title">
    ...
    <header className="ai-header"><div><div className="ai-title-row"><h1 id="ac-title">Completed assembly</h1><span className="ai-demo">Submitted orders</span></div><p>Review boxes submitted after assembly and quality checks.</p></div><button type="button" className="ai-refresh" onClick={reload} disabled={loading}><RefreshCw size={16} aria-hidden="true" />Refresh</button></header>
    ...
    <section className="ai-panel" aria-labelledby="ac-list-title">
      ...
      {visible.length > 0 && <div className="ai-table-scroll" tabIndex={0} role="region" aria-label="Submitted assembly orders"><table><thead><tr><th scope="col">Order</th><th scope="col">Occasion</th><th scope="col">Box size</th><th scope="col">Submitted</th><th scope="col">Status</th><th scope="col">Action</th></tr></thead><tbody>{visible.map(order => <tr key={order.id}>
        ...
        <td>{order.submittedAt && Number.isFinite(Date.parse(order.submittedAt)) ? <time dateTime={order.submittedAt}>{new Date(order.submittedAt).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}</time> : <span>Not recorded<small>Submission date unavailable</small></span>}</td>
    ...
    <p className="ai-footnote">Submissions are saved in the database. Final delivery approval belongs to the admin.</p>
  </section>;
}
